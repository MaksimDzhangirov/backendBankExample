...
instruct Gin that it should run the server in release mode instead. So let's
stop the server and try it! I'm gonna remove the old `simplebank` container,
...
And voila, the server has started, but now we don't see any debug logs as 
...
Voila, the IP address of the `simplebank` container is different: 
...
and voila, the server is now started successfully.
